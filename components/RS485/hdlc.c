...
unsigned short crc_itu16_append(unsigned char *buf, unsigned short len)
{
    ...
    while (len-- > 0)
        crc = (crc >> 8) ^ crc_itu16_table[(crc ^ *buf++) & 0xFF];
    crc ^= 0xFFFF;
    *buf++ = (unsigned char) crc;
    *buf++ = (unsigned char) (crc >> 8);
    return new_len;
}
...
void hdlc_rx_byte(hdlc_rx_state_t *s, unsigned char new_byte)
{
    ...
    for (i = 0;  i < 8;  i++)
    {
        ...
        {
            if (s->flag_seen && (s->bit_buf & 0x3F00) != 0x3E00)
            {
                s->byte_in_progress = (s->byte_in_progress | (s->bit_buf & 0x0100)) >> 1;
                if (++s->num_bits == 8)
                {
                    ...
                    {
                        s->buffer[s->len++] = (unsigned char) s->byte_in_progress;
                    }
                    s->num_bits = 0;
                }
            }
...
void hdlc_tx_frame(hdlc_tx_state_t *s, unsigned char *frame, 
                   unsigned short len)
{
    unsigned long forming;
    unsigned short crc; 
    unsigned char bits;
    unsigned char byte_in_progress;
    int i, j;
 
    crc = crc_itu16_calc(frame, len, 0xFFFF) ^ 0xFFFF;

    /* Complete the flag octet currently in progress */
    forming = s->idle_byte >> (unsigned char)(8 - s->num_bits);
    bits = s->num_bits;
    byte_in_progress = 0;

    while (len--)
    {
        byte_in_progress = *frame++;
        for (i = 0; i < 8; i++)
        {
        ...
        s->buffer[s->len++] = (unsigned char) (forming >> bits);
        if (bits >= 8)
        {
            bits -= 8;
            s->buffer[s->len++] = (unsigned char) (forming >> bits);
    ...
    s->buffer[s->len++] = (unsigned char) ((forming << (8 - bits)) | (0x7E >> bits));
    /* Now create a full byte of flag */
    s->idle_byte = (0x7E7E >> bits) & 0xFF;
    s->buffer[s->len++] = (unsigned char)s->idle_byte;
    s->num_bits = bits;
}
...
unsigned char hdlc_tx_getbyte(hdlc_tx_state_t *s)
{
    ...
    {
        txbyte = (unsigned char)s->idle_byte;
    }
    return txbyte;
}
