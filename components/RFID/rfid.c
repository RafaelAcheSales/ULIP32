...
static uint32_t rfid_timestamp;
static int rfid_panic_timestamp = 0;
...
static int llcp_wait_connection(void)
{
    ...
    os_info("RFID", "LLCP wait connection");
...
static int llcp_read(uint8_t *data, int len)
{
    ...
    os_info("RFID", "SNEP TNF [%d] RTD [%d] payload size [%d]",
            tnf, type, len);

    rfid_timestamp = system_get_time();
    /* RTD Text */
    rc = -1;
    if (type == 'T') {
        /* Status */
        ...
        if (len < sizeof(key)) {
            ...
            os_debug("RFID", "SNEP invalid RTD size!");
        }
    } else {
        os_debug("RFID", "SNEP invalid RTD!");
    }
                       ...
                       int len, void *user_data)
{
...
#ifdef DEBUG
    os_debug("RFID", "RFID read [%d] bytes len [%d]",
             len, rfid_buflen);
#endif

    if (rfid_buflen + len > RFID_BFSIZE) {
#ifdef DEBUG
        os_warning("RFID", "RFID buffer overflow");
#endif
        rfid_buflen = 0;
        return;
    }
    os_memcpy(rfid_buf + rfid_buflen, event, len);
    rfid_buflen += len;

    /* Check packet size */
    if (rfid_buflen < 6) return;
    ...
    while (rfid_buflen >= 6) {
...
#ifdef DEBUG
            os_warning("RFID", "Invalid header!");
#endif
            rfid_buflen = 0;
            break;
        }
        ...
        if (cmd == PN532_ACK) {
            ...
            if (crc == 0) {
...
#ifdef DEBUG
                    os_warning("RFID", "Invalid command!");
#endif
                    rfid_buflen = 0;
                    break;
                }
                ...
                if (crc != 0) {
#ifdef DEBUG
                    os_warning("RFID", "Checksum error!");
#endif
                    rfid_buflen = 0;
                    break;
                }
                ...
                switch (cmd) {
                    case PN532_COMMAND_SAMCONFIGURATION:
                        os_info("RFID", "SAM configuration response");
                        rfid_module_initialize(++rfid_init_stage);
                        break;
                    case PN532_COMMAND_RFCONFIGURATION:
                        os_info("RFID", "RF configuration response");
                        rfid_module_initialize(++rfid_init_stage);
                        break;
                    /* MIFARE Classic */
                    case PN532_COMMAND_INLISTPASSIVETARGET:
                        ...
                        os_sprintf(card, "%lu", serial);
                        /* Debounce */
                        now = system_get_time();
                        if (rfid_timestamp) {
                            if (!os_strcmp(rfid_card, card)) {
                                d = (now - rfid_timestamp) / 1000;
                                if (d <= (rfid_timeout << 1)) {
                                    os_debug("RFID", "Debounce card [%s]", card);
                                    /* Panic */
                                    if (rfid_panic_timestamp > 0) {
                                        ...
                                        if (rfid_panic_timestamp <= 0) {
                                            rc = rfid_func(RFID_EVT_CHECK, card, len,
                                                           rfid_user_data);
                                            if (!rc) {
                                                os_info("RFID", "Panic card [%s]", card);
                        ...
                        os_strncpy(rfid_card, card, sizeof(rfid_card) - 1);
                        rfid_timestamp = now;
                    ...
                    case PN532_COMMAND_TGGETDATA:
                        ...
                        if (!error) {
                            ...
                            /* Error */
#ifdef DEBUG
                            os_debug("RFID", "LLCP error [0x%02X]", error);
#endif
                            if (error == 0x25) {
                    ...
                    default:
#ifdef DEBUG
                        os_warning("RFID", "Invalid command [0x%02X]", cmd);
#endif
                        break;
                }
                rfid_buflen -= size + 7;
...
#ifdef DEBUG
                os_warning("RFID", "Invalid frame length!");
#endif
                rfid_buflen = 0;
            }
            break;
    ...
    if (rfid_buflen > 0) {
        if (p.b != rfid_buf)
            os_memcpy(rfid_buf, p.b, rfid_buflen);
    }
}
...
#ifdef DEBUG
    os_debug("RFID", "Send command [%d] bytes", len);
#endif
...
static void rfid_module_initialize(int stage)
{
    ...
    os_debug("RFID", "RFID init stage [%d]", stage);
    ...
    if (llcp_state == LLCP_DISCONNECTED) {
        ...
        if (!rfid_configured) {
            ...
            if (rfid_status++ >= RFID_NUM_STATUS) {
                os_warning("RFID", "RFID module restart");
                /* Initialize module */
                rfid_module_initialize(0);
                return;
            }
...
{
    os_info("RFID", "Initialize RFID");

    /* Open tty */
    if (tty_open(RFID_TTY, rfid_event, NULL)) {
        os_warning("RFID", "Failed to initialize RFID!");
        return -1;
    }
    rfid_func = func;
    ...
    rfid_format = format;
    os_memset(rfid_card, 0, sizeof(rfid_card));
    rfid_timestamp = 0;
    ...
    // os_timer_setfn(&rfid_timer, (os_timer_func_t *)rfid_polling_timeout, NULL);
    // os_timer_arm(&rfid_timer, rfid_timeout, TRUE);
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(rfid_timer, rfid_timeout));
    return 0;
}
