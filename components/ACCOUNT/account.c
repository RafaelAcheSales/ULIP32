...
//                         if (!p) continue;
//                         m2 = strtol(p, NULL, 10);
//                         os_sprintf(date, "%04d/%02d/%02d %02d:%02d",
//                                    y2, t2, d2, h2, m2);
//                         if (os_strcmp(now, date) < 0) {
//                             rv = FALSE;
//                             break;
//                         }
//                     }
...
//     data = (uint8_t *)os_malloc(DB_SECTOR_SIZE);
//     if (!data) return -1;
...
//     /* Visitor */
//     os_timer_setfn(&account_db.timer,
//                    (os_timer_func_t *)account_db_cleanup, NULL);
//     os_timer_arm(&account_db.timer, 300000, TRUE);
...
// {
//     os_timer_disarm(&account_db.timer);
...
//     acc = (account_t *)os_zalloc(sizeof(account_t));
//     if (!acc) return NULL;
...
//     os_memset(acc->a.name, 0, sizeof(acc->a.name));
//     if (name)
//         os_strncpy(acc->a.name, name,
//                    sizeof(acc->a.name) - 1);
// }
...
//     os_memset(acc->a.user, 0, sizeof(acc->a.user));
//     if (user)
//         os_strncpy(acc->a.user, user,
//                    sizeof(acc->a.user) - 1);
// }
...
//     os_memset(acc->a.password, 0, sizeof(acc->a.password));
//     if (password)
//         os_strncpy(acc->a.password, password,
...
//     os_memset(acc->a.card, 0, sizeof(acc->a.card));
//     if (card)
//         os_strncpy(acc->a.card, card, sizeof(acc->a.card) - 1);
// }
...
//     os_memset(acc->a.code, 0, sizeof(acc->a.code));
//     if (code)
//         os_strncpy(acc->a.code, code, sizeof(acc->a.code) - 1);
// }
...
//     os_memset(acc->a.fingerprint, 0, sizeof(acc->a.fingerprint));
//     if (fingerprint)
//         os_memcpy(acc->a.fingerprint, fingerprint,
...
//     os_memset(acc->a.perm, 0, sizeof(acc->a.perm));
//     if (perm) {
//         for (i = 0; i < len && i < ACCOUNT_PERMISSIONS; i++) {
//             if (*perm[i] != '\0')
//                 os_strcpy(acc->a.perm[size++], perm[i]);
//         }
//     }
// }
...
//     os_memset(acc->a.rfcode, 0, sizeof(acc->a.rfcode));
//     if (rfcode)
//         os_strncpy(acc->a.rfcode, rfcode, sizeof(acc->a.rfcode) - 1);
// }
...
//     os_memset(acc->a.key, 0, sizeof(acc->a.key));
//     if (key)
//         os_strncpy(acc->a.key, key, sizeof(acc->a.key) - 1);
// }
...
//     os_memset(acc->a.finger, 0, sizeof(acc->a.finger));
//     if (finger)
//         os_strncpy(acc->a.finger, finger,
...
//     min = tm->tm_min; 
//     wday = tm->tm_wday;
//     os_sprintf(now, "%04d/%02d/%02d %02d:%02d",
//                year, mon, mday, hour, min); 
//     for (j = 0; *acc->a.perm[j] != '\0' && j < ACCOUNT_PERMISSIONS; j++) {
//         os_strcpy(period, acc->a.perm[j]);
//         if (!os_strchr(period, '/')) {
//             /*
...
//         os_memcpy(data + off, acc, sizeof(account_t));
//         ets_intr_lock();
...
//         system_soft_wdt_feed();
//         os_free(data);
//         os_debug("DB", "Database update node [%d] empty [%d %d] full [%d %d]",
...
//     os_debug("DB", "Database insert node [%d] empty [%d %d] full [%d %d]",
//              index, account_db.node_empty, account_db.node_index_empty,
//              account_db.node_full, account_db.node_index_full);
...
//         return -1;
//     }
//     os_memset(data + off, 0, DB_NODE_SIZE);
//     /* Update database node */
//     ets_intr_lock();
//     spi_flash_erase_sector(sector);
//     spi_flash_write(addr, (uint32 *)data, DB_SECTOR_SIZE);
...
//     os_debug("DB", "Database remove node [%d] empty [%d %d] full [%d %d]",
//              index, account_db.node_empty, account_db.node_index_empty,
//              account_db.node_full, account_db.node_index_full);
...
//                 (user && *user && !os_strcmp(user, acc.a.user)) ||
//                 (card && *card && !os_strcmp(card, acc.a.card)) ||
//                 (code && *code && !os_strcmp(code, acc.a.code)) ||
//                 (rfcode && *rfcode && !os_strcmp(rfcode, acc.a.rfcode)) ||
//                 (fingerprint && *fingerprint &&
//                  !os_memcmp(fingerprint, acc.a.fingerprint, ACCOUNT_FINGERPRINT_SIZE)) ||
//                 (key && *key && !os_strcmp(key, acc.a.key)))
//                 return i;
//         }
//         addr += DB_NODE_SIZE;
//         /* Watchdog */
...
//             for (k = 0; k < 3; k++) {
//                 os_snprintf(stmp, sizeof(stmp), "%s:%lu",
//                             acc.a.key, d + k - 1);
//                 SHA1_Init(&ctx);
//                 SHA1_Update(&ctx, stmp, strlen(stmp));
//                 SHA1_Final(hash, &ctx);
//                 for (j = 0; j < SHA1_DIGEST_LENGTH; j++)
//                     os_snprintf(stmp + (j << 1), sizeof(stmp) - (j << 1),
//                                 "%02x", hash[j]);
...
//     acc = (account_t *)os_malloc(sizeof(account_t));
//     if (!acc) return NULL;
...
//     else
//         temp[0] = '\0';
//     size = os_snprintf(json, len, "{\"id\":\"%d\",\"name\":\"%s\",\"user\":\"%s\"," 
...
//         size += os_snprintf(json + size, len - size, ",\"perm%d\":\"%s\"",
//                             i + 1, acc->a.perm[i]);
//     if (size + 2 > len) {
//         os_free(acc);
//         return -1;
...
//     len = os_snprintf(json, len, "{\"accounts\":\"%d\",\"user\":\"%d\"," 
//                       "\"card\":\"%d\",\"qrcode\":\"%d\"," 
...
//     else
//         temp[0] = '\0';
//     size = os_snprintf(str, len, "%s,%s,%s,%s,%s,%s,%s,%d,%d,%d,%s,%d,%d,%c",
...
//                        acc->a.finger);
//     for (i = 0; i < ACCOUNT_PERMISSIONS; i++)
//         size += os_snprintf(str + size, len - size, ",%s",
//                             acc->a.perm[i]);
//     if (size + 3 > len) {
//         os_free(acc);
//         return -1;
...
//     p = (account_log_t *)os_zalloc(sizeof(account_log_t));
...
//     os_memset(log->date, 0, sizeof(log->date));
//     if (date)
//         os_strncpy(log->date, date, sizeof(log->date) - 1);
// }
...
//     os_memset(log->name, 0, sizeof(log->name));
//     if (name)
//         os_strncpy(log->name, name, sizeof(log->name) - 1);
// }
...
//     os_memset(log->code, 0, sizeof(log->code));
//     if (code)
//         os_strncpy(log->code, code, sizeof(log->code) - 1);
// }
...
//     os_memset(&d, 0, sizeof(account_log_data_t));
//     d.a.timestamp = rtc_mklocaltime(rtc_localtime());
//     os_strncpy(d.a.name, log->name, sizeof(d.a.name) - 1);
//     os_strncpy(d.a.data, log->code, sizeof(d.a.data) - 1);
//     d.a.type = log->type;
//     d.a.granted = log->granted;
//     os_memcpy(data + off, &d, sizeof(account_log_data_t));
//     ets_intr_lock();
//     spi_flash_erase_sector(sector);
//     spi_flash_write(addr, (uint32 *)data, DB_SECTOR_SIZE);
//     ets_intr_unlock();
...
//     os_debug("DB", "Database log write node [%d]", index);
...
//     log = (account_log_t *)os_zalloc(sizeof(account_log_t));
//     if (!log) return NULL;
//     tm = rtc_gmtime(data.a.timestamp);
//     os_sprintf(log->date, "%04d-%02d-%02d %02d:%02d:%02d",
//                tm->tm_year, tm->tm_mon + 1, tm->tm_mday,
//                tm->tm_hour, tm->tm_min, tm->tm_sec);
//     os_strncpy(log->name, data.a.name, sizeof(log->name) - 1);
//     os_strncpy(log->code, data.a.data, sizeof(log->code) - 1);
...
//     size = os_snprintf(str, len, "%s,%s,%s,%s",
...
//             break;
//     }
//     size = os_snprintf(json, len, "{\"name\":\"%s\",\"user\":\"%s\",\"card\":\"%s\"," 
