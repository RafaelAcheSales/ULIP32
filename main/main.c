...
static int ulip_core_httpd_request(httpd_req_t *req)
{
    char request[128] = "";
    char state[32] = "";
    char url[256] = "";
...
// #endif
    ...
    return ESP_OK;
#if 0
        else if (!os_strcmp(request, "status"))
        {
            /* JSON */
            body = (char *)os_malloc(256);
            if (!body)
            {
        ...
        else if (!os_strcmp(request, "relay"))
        {
            ...
            if (!os_strcmp(state, "on") || !os_strcmp(state, "hold"))
            {
                if (!CFG_get_control_external())
                {
                    /* Local control */
                    if (!os_strcmp(state, "on"))
                    {
                        if (CFG_get_control_mode() == CFG_CONTROL_MODE_AUTO)
                            ctl_relay_on(CFG_get_control_timeout());
                        else
    ...
    !defined(__MLI_1WLC_TYPE__)
        ...
        else if (!os_strcmp(request, "alarm"))
        {
            httpdFindArg(connData->getArgs, "state",
                         state, sizeof(state));
            if (!os_strcmp(state, "on"))
        ...
        else if (!os_strcmp(request, "panic"))
        {
            httpdFindArg(connData->getArgs, "state",
                         state, sizeof(state));
            if (!os_strcmp(state, "on"))
        ...
        else if (!os_strcmp(request, "update"))
        {
        ...
        else if (!os_strcmp(request, "reboot"))
        {
            httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
            httpdStartResponse(connData, 200);
            httpdHeader(connData, "Content-Length", "0");
            httpdEndHeaders(connData);
            os_timer_setfn(&reboot_timer,
                           (os_timer_func_t *)ulip_core_reboot, NULL);
            os_timer_arm(&reboot_timer, 1000, false);
            return HTTPD_CGI_DONE;
        }
        else if (!os_strcmp(request, "setconfig"))
        {
            ...
            while ((p = strtok(config, ",")))
            {
                ...
                strdelimit(p, "{}\r\n", ' ');
                strstrip(p);
                if (!os_strncmp("\"hotspot\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_hotspot(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"ap_mode\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_ap_mode(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"standalone\":", p, 13))
                {
                    p += 13;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_standalone(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"ssid\":", p, 7))
                {
                    p += 7;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"password\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"channel\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"beacon_interval\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"ssid_hidden\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_ssid_hidden(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"dhcp\":", p, 7))
                {
                    p += 7;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_dhcp(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"ip\":", p, 5))
                {
                    p += 5;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"netmask\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"gateway\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"hostname\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"timezone\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dst\":", p, 6))
                {
                    p += 6;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_dst(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"dst_date\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"server_port\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"server_user\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"server_password\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"server_url\":", p, 13))
                {
                    p += 13;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"server_retries\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"user_auth\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_user_auth(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"ota_url\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rfid\":", p, 7))
                {
                    p += 7;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_rfid_enable(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"rfid_timeout\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rfid_panic_timeout\":", p, 21))
                {
                    p += 21;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rfid_nfc\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_rfid_nfc(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"rfid_panic_timeout\":", p, 21))
                {
                    p += 21;
                ...
                else if (!os_strncmp("\"rfid_retries\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"qrcode\":", p, 9))
                {
                    p += 9;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_qrcode_enable(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"qrcode_timeout\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"qrcode_dynamic\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"qrcode_validity\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"qrcode_panic_timeout\":", p, 23))
                {
                    p += 23;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"qrcode_config\":", p, 16))
                {
                    p += 16;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_qrcode_config(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"control_external\":", p, 19))
                {
                    p += 19;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_control_external(!os_strcmp(p, "true"));
                }
                else if (!os_strncmp("\"control_url\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"control_mode\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"control_timeout\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"control_description\":", p, 22))
                {
                    p += 22;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"control_acc_timeout\":", p, 22))
                {
                    p += 22;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"control_doublepass_timeout\":", p, 29))
                {
                    p += 29;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rs485\":", p, 8))
                {
                    p += 8;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_rs485_enable(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"rs485_address\":", p, 16))
                {
                    p += 16;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rs485_server_address\":", p, 23))
                {
                    p += 23;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"debug\":", p, 8))
                {
                    ...
                    case DEBUG_MODE_SERIAL:
                        ESP_LOGD_enable();
                        ESP_LOGD_set_level(level);
                        ESP_LOGD_set_dump_serial();
                        break;
                    case DEBUG_MODE_NETWORK:
                        ESP_LOGD_enable();
                        ESP_LOGD_set_level(level);
                        ESP_LOGD_set_dump_network(server, port);
                        break;
                    }
                    CFG_set_debug(mode, level, server, port);
                }
                else if (!os_strncmp("\"rf433\":", p, 8))
                {
                    p += 8;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_rf433_enable(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"fingerprint\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_fingerprint_enable(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"fingerprint_timeout\":", p, 22))
                {
                    p += 22;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"fingerprint_security\":", p, 23))
                {
                    p += 23;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"fingerprint_retries\":", p, 22))
                {
                    p += 22;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"latitude\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"longitude\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"watchdog_shutdown\":", p, 20))
                {
                    p += 20;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"ddns\":", p, 7))
                {
                    p += 7;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_ddns(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"ddns_domain\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"ddns_user\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"ddns_password\":", p, 16))
                {
                    p += 16;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rf433_rc\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_rf433_rc(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"rf433_hc\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rf433_bc\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_rf433_bc(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"rf433_bp\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rf433_panic_timeout\":", p, 22))
                {
                    p += 22;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"rf433_ba\":", p, 11))
                {
                    p += 11;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"lora\":", p, 7))
                {
                    p += 7;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                    CFG_set_lora_enable(!os_strcmp(p, "on"));
                }
                else if (!os_strncmp("\"lora_channel\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"lora_baudrate\":", p, 16))
                {
                    p += 16;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"lora_address\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"lora_server_address\":", p, 22))
                {
                    p += 22;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_timeout\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_temp_upper\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_temp_lower\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_rh_upper\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_rh_lower\":", p, 15))
                {
                    p += 16;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_relay\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"dht_alarm\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"mq2_timeout\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"mq2_limit\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"mq2_relay\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"mq2_alarm\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pir_chime\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pir_relay\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pir_alarm\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pir_timeout\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"sensor_type\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"sensor_flow\":", p, 14))
                {
                    p += 14;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"sensor_limit\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"temt_timeout\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"temt_upper\":", p, 13))
                {
                    p += 13;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"temt_lower\":", p, 13))
                {
                    p += 13;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"temt_relay\":", p, 13))
                {
                    p += 13;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"temt_alarm\":", p, 13))
                {
                    p += 13;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"relay_status\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_voltage_cal\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_voltage_upper\":", p, 20))
                {
                    p += 20;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_voltage_lower\":", p, 20))
                {
                    p += 20;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_current_cal\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_current_upper\":", p, 20))
                {
                    p += 20;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_current_lower\":", p, 20))
                {
                    p += 20;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_power_upper\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_power_lower\":", p, 18))
                {
                    p += 18;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_relay\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_alarm_time\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_relay_timeout\":", p, 20))
                {
                    p += 20;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_relay_ext\":", p, 16))
                {
                    p += 16;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_interval\":", p, 15))
                {
                    p += 15;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_day\":", p, 10))
                {
                    p += 10;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"pow_energy_cal\":", p, 17))
                {
                    p += 17;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"energy_daily_limit\":", p, 21))
                {
                    p += 21;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"energy_monthly_limit\":", p, 23))
                {
                    p += 23;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
                ...
                else if (!os_strncmp("\"energy_total_limit\":", p, 21))
                {
                    p += 21;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
        ...
        else if (!os_strcmp(request, "getconfig"))
        {
            uint8_t mode = 0;
            uint8_t level = 0;
    ...
    defined(__MLI_1WRC_TYPE__) || defined(__MLI_1WLC_TYPE__)
        ...
        else if (!os_strcmp(request, "restoreconfig"))
        {
            ulip_core_restore_config(TRUE);
        }
        else if (!os_strcmp(request, "users"))
        {
        ...
        else if (!os_strcmp(request, "usersummary"))
        {
    ...
    !defined(__MLI_1WLC_TYPE__)
        ...
        else if (!os_strcmp(request, "accesslog"))
        {
            httpdFindArg(connData->getArgs, "file", file, sizeof(file));
            /* Check filter */
            if (connData->post && connData->post->buff)
            ...
            if (start && end)
            {
                ...
                if (log)
                {
                    if (os_strcmp(end, account_log_get_date(log)) < 0)
                    {
                        if (connData->cgiData)
                        {
                    ...
                    if (os_strcmp(start, account_log_get_date(log)) > 0)
                    {
                        if (*file == '\0')
                            httpdSendData(connData, "]", 1);
                        os_free(log);
        ...
        else if (!os_strcmp(request, "delaccesslog"))
        {
        ...
        else if (!os_strcmp(request, "adduser"))
        {
#if defined(__MLI_1WB_TYPE__) || defined(__MLI_1WQB_TYPE__)
            if (connData->cgiData)
            {
            ...
            if (index != -1)
            {
                ...
                if (name && os_strcmp(name, account_get_name(acc) ? account_get_name(acc) : ""))
                {
                    update = TRUE;
                }
                else if (user && os_strcmp(user, account_get_user(acc) ? account_get_user(acc) : ""))
                {
                    update = TRUE;
                }
                else if (password && os_strcmp(password, account_get_password(acc) ? account_get_password(acc) : ""))
                {
                    update = TRUE;
                }
                else if (card && os_strcmp(card, account_get_card(acc) ? account_get_card(acc) : ""))
                {
                    update = TRUE;
                }
                else if (qrcode && os_strcmp(qrcode, account_get_code(acc) ? account_get_code(acc) : ""))
                {
                    update = TRUE;
                }
                else if (rfcode && os_strcmp(rfcode, account_get_rfcode(acc) ? account_get_rfcode(acc) : ""))
                {
                    update = TRUE;
                }
                ...
                else if (key && os_strcmp(key, account_get_key(acc) ? account_get_key(acc) : ""))
                {
                    update = TRUE;
                }
                ...
                else if (finger && os_strcmp(finger, account_get_finger(acc) ? account_get_finger(acc) : ""))
                {
                    update = TRUE;
                }
                else
        ...
        else if (!os_strcmp(request, "getuser"))
        {
            if (!connData->post || !connData->post->buff)
            {
                httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
        ...
        else if (!os_strcmp(request, "deluser"))
        {
            if (!connData->post || !connData->post->buff)
            {
                httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
        ...
        else if (!os_strcmp(request, "probeuser"))
        {
            httpdFindArg(connData->getArgs, "state",
                         state, sizeof(state));
            /* JSON */
        ...
        else if (!os_strcmp(request, "eraseuser"))
        {
            httpdFindArg(connData->getArgs, "state",
                         state, sizeof(state));
            if (!os_strcmp(state, "off"))
        ...
        else if (!os_strcmp(request, "checkuser"))
        {
            if (!connData->post || !connData->post->buff)
            {
                httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
        ...
        else if (!os_strcmp(request, "eraseall"))
        {
            account_db_remove_all();
#if defined(__MLI_1WB_TYPE__) || defined(__MLI_1WQB_TYPE__)
            fpm_delete_all();
        ...
        else if (!os_strcmp(request, "finger"))
        {
            if (!connData->post || !connData->post->buff)
            {
                httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
        ...
        else if (!os_strcmp(request, "location"))
        {
            /* JSON */
            body = (char *)os_malloc(256);
            if (!body)
        ...
        else if (!os_strcmp(request, "alarms"))
        {
            /* JSON */
            body = (char *)os_malloc(256);
            if (!body)
        ...
        else if (!os_strcmp(request, "getdatetime"))
        {
            /* JSON */
            body = (char *)os_malloc(256);
            if (!body)
        ...
        else if (!os_strcmp(request, "setdatetime"))
        {
            if (!connData->post || !connData->post->buff)
            {
                httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
            ...
            while ((p = strtok(config, ",")))
            {
                ...
                else if (!os_strncmp("\"timestamp\":", p, 12))
                {
                    p += 12;
                    strdelimit(p, "\"", ' ');
                    strstrip(p);
    ...
    defined(__MLI_1WQF_TYPE__) || defined(__MLI_1WRQ_TYPE__)
        ...
        else if (!os_strcmp(request, "getqrcode"))
        {
            /* Check account */
        ...
        else if (!os_strcmp(request, "beep"))
        {
            httpdFindArg(connData->getArgs, "state",
                         state, sizeof(state));
            if (!os_strcmp(state, "granted") || !os_strcmp(state, "panic"))
                ctl_beep(0);
            else if (!os_strcmp(state, "blocked"))
                ctl_buzzer_on(CTL_BUZZER_ERROR);
            httdSetTransferMode(connData, HTTPD_TRANSFER_CLOSE);
            httpdStartResponse(connData, 200);
            httpdHeader(connData, "Content-Length", "0");
    ...
    defined(__MLI_1WRP_TYPE__) || defined(__MLI_1WRC_TYPE__) || \
    defined(__MLI_1WLC_TYPE__)
        else if (!os_strcmp(request, "telemetry"))
        {
            /* JSON */
            body = (char *)os_malloc(512);
            if (!body)
        ...
        else if (!os_strcmp(request, "deltelemetry"))
        {
            sensor_cycles = 0;
            if (CFG_get_sensor_cycles())
            {
        ...
        else if (!os_strcmp(request, "telemetrylog"))
        {
            httpdFindArg(connData->getArgs, "file", file, sizeof(file));
            /* Check filter */
            if (connData->post && connData->post->buff)
            ...
            if (start && end)
            {
                ...
                if (log)
                {
                    if (os_strcmp(end, telemetry_get_date(log)) < 0)
                    {
                        if (connData->cgiData)
                        {
                            httpdSuspend(connData, 10);
                    ...
                    if (os_strcmp(start, telemetry_get_date(log)) > 0)
                    {
                        if (*file == '\0')
                            httpdSendData(connData, "]", 1);
                        os_free(log);
        ...
        else if (!os_strcmp(request, "deltelemetrylog"))
        {
        ...
        else if (!os_strcmp(request, "relayaux"))
        {
            httpdFindArg(connData->getArgs, "state",
                         state, sizeof(state));
            if (!os_strcmp(state, "on") || !os_strcmp(state, "hold"))
